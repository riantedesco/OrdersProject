...
public class ProductOrderServiceImpl implements ProductOrderService {
	...
	public ProductOrderDto save(ProductOrderFormDto body, OrderEntity order) {
		...
		if (!product.get().getActive()) {
			throw new InactiveProductException("Inactive product");
		}
		productOrder.setProduct(product.get());
		productOrder.setOrder(order);
		productOrder.setTotal(body.getQuantity() * product.get().getPrice());

		validation.validateSaveProductOrder(productOrder);
		ProductOrderEntity response = this.productOrderRepository.save(productOrder);
		return mapper.map(response, ProductOrderDto.class);
	}
